...
def fillObsCPT(bayesNet, gameState):
    ...
    for pos in gameState.getPossibleHouses():
        for wall in gameState.getHouseWalls(pos):
            var = OBS_VAR_TEMPLATE % wall
            obsFac = bn.Factor([var], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for temp in obsFac.getAllPossibleAssignmentDicts():
                foodVar = temp[FOOD_HOUSE_VAR]
                ghostVar = temp[GHOST_HOUSE_VAR]
                colorVar = temp[var]
                probVar = 0
                ...
                if pos == topLeftPos:
                    posVar = 'topLeft'
                elif pos == bottomRightPos:
                    posVar = 'bottomRight'
                elif pos == topRightPos:
                    posVar = 'topRight'

                
                if foodVar == posVar:
                    if colorVar == RED_OBS_VAL:
                        probVar = PROB_FOOD_RED
                    elif colorVar == BLUE_OBS_VAL:
                        probVar = 1-PROB_FOOD_RED
                    else:
                        probVar = 0
                elif ghostVar == posVar:
                    if colorVar == RED_OBS_VAL:
                        probVar = PROB_GHOST_RED
                    elif colorVar == BLUE_OBS_VAL:
                        probVar = 1-PROB_GHOST_RED
                    else:
                        probVar = 0
                else:
                    if colorVar == NO_OBS_VAL:
                        probVar = 1
                    else:
                        probVar = 0

                obsFac.setProbability(temp, probVar)
            bayesNet.setCPT(var, obsFac)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    best = None
    prob = 0
    for var in factor.getAllPossibleAssignmentDicts():
        temp = factor.getProbability(var)
        if temp > prob:
            prob, best = temp, var
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for temp in factor.getAllPossibleAssignmentDicts():
            if temp[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and temp[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                rightExpectedValue = factor.getProbability(temp) * WON_GAME_REWARD + (1  - factor.getProbability(temp)) * GHOST_COLLISION_REWARD

            elif temp[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and temp[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExpectedValue = factor.getProbability(temp) * WON_GAME_REWARD + (1  - factor.getProbability(temp)) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, explore in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue += prob * max(self.computeEnterValues(explore, enterEliminationOrder))
